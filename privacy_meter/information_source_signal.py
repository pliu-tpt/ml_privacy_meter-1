...
########################################################################################################################
# ModelSingleStepPGD CLASS
########################################################################################################################

class ModelSingleStepPGD(Signal):
    """
    Inherits from the Signal class, used to represent any type of signal that can be obtained from a Model and/or a
    Dataset.
    This particular class is used to get the pgd loss trajectories of a model.
    """
    ...
    def __call__(
        ...
        Returns:
            The signal value.
        """
        print(25*"X"+"COMPUTING PGD") # TOREVERT
        ...
        for k, model in enumerate(models):
            ...
            x = datasets[dataset_index].get_feature(split_name, input_feature)
            y = datasets[dataset_index].get_feature(split_name, output_feature)
            results.append(model.get_single_pgd_percentile(x, y, **extra)) # TODO; unrestricted eps option using extra  eps=1, alpha=2/255, iters=40, nb_perturbed=1, noise_range=0
        return results
