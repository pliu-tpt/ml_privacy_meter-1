...
#                Dataloader                 #
#############################################
def get_cifar10_data(dataset, train_index, test_index, device):
...
def make_net(data, device, hyp=hyp, depths=depths):
...
def fast_train_fun(data, net, device, hyp=hyp, batchsize=batchsize, eval_batchsize = 2500):
...
# if __name__ == "__main__":
    ...
    for run_num in range(1):
        ...
        net = make_net(data, device=device)
        print_training_details(
        ...
        loss_fn = nn.CrossEntropyLoss(label_smoothing=0.2, reduction="none")
        net_ema,train_acc, train_loss, ema_val_acc, val_loss = fast_train_fun(data, net, device=device)
        loss_list_all = []
